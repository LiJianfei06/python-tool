...
def ResBlock(network,last_name='',block_name='conv2_',block_n=3,num_out=16,downsampling=False,down_method='pooling',use_global_stats='False'):
    ...
    for i in range(1,block_n+1,1):
        ...
        network,last_name=write_prototxt.BatchNorm(network,name_bn=block_name+str(i)+"bn"+'_1',name_scale=block_name+str(i)+"_scale"+'1',
...
def Net( mode='train',root_path='',batch_size=32):
    ...
    if mode=='train':
        network,last_name=write_prototxt.data(network,name="data",mirror=True,scale=0.00390625,crop_size=64,batch_size=batch_size,backend="LMDB",shuffle=True,datasets_path=root_path)
        use_global_stats=False
    elif mode=='test':
        network,last_name=write_prototxt.data(network,name="data",mirror=False,scale=0.00390625,crop_size=64,batch_size=batch_size,backend="LMDB",shuffle=False,datasets_path=root_path)
        use_global_stats=True
    
    
    network,last_name=write_prototxt.Convolution(network,name="conv1",bottom_name=last_name,top_name='conv1',num_output=24,
                        bias_term=True,pad=1,kernel_size=3,stride=1,weight_type='msra',bias_type='constant')
    ...
    network,last_name=ResBlock(network,last_name=last_name,block_name='conv2_',block_n=9,num_out=24,downsampling=False,use_global_stats=use_global_stats)
    network,last_name=ResBlock(network,last_name=last_name,block_name='conv3_',block_n=9,num_out=32,downsampling=True,down_method='conv',use_global_stats=use_global_stats)
    network,last_name=ResBlock(network,last_name=last_name,block_name='conv4_',block_n=9,num_out=64,downsampling=True,down_method='conv',use_global_stats=use_global_stats)
    network,last_name=ResBlock(network,last_name=last_name,block_name='conv5_',block_n=9,num_out=128,downsampling=True,down_method='conv',use_global_stats=use_global_stats)
    ...
    network,last_name=write_prototxt.InnerProduct(network,name="fc1",bottom_name=last_name,top_name='fc1',num_output=230,weight_type='xavier',bias_type='constant')
    if mode=='train':
...
if __name__ == '__main__':
    root_path_train="/home/lijianfei/datasets/tianchi_Zero_samples/train_lmdb"
    root_path_test="/home/lijianfei/datasets/tianchi_Zero_samples/test_lmdb"
    ...
    with open("test.prototxt", 'w') as f:
        f.write(str(Net(mode='test',root_path=root_path_test,batch_size=1)))#创建 train.prototxt
